...
class Logger {
    ...
    private formatter?: (message: string, level: LogLevel, timestamp?: Date) => string;
    ...
    private log(message: string, level: LogLevel) {
        ...
        if (this.formatter) {
            output = this.formatter(message, level, this.timestamp ? new Date() : undefined);
        } else {
            // Default formatting
            ...
            if (!this.emoji) {
                ...
                output = output.replace(/[\u{1F300}-\u{1F6FF}\u{1F900}-\u{1F9FF}\u{1F1E6}-\u{1F1FF}\u{2600}-\u{26FF}\u{2700}-\u{27BF}]/gu, '');
            }
        }
